...
setup_caddy_env() {
    ...
    echo ""
    log_info "Configure projects that Caddy will serve"
    log_info "Each project needs: domain (required), network (required), caddyfile (optional)"
    echo ""
    ...
    while true; do
        ...
        echo ""
        if ! prompt_yes_no "Add another project?" "n"; then
            break
        fi
    done
