...
# 2. Create .env and projects.env from templates
...
PROJECTS_EXAMPLE="${SCRIPT_DIR}/projects.env.example"
...
setup_projects_file() {
    log_step "Setting up projects.env file"

    if [ -f "$PROJECTS_FILE" ]; then
        log_warn "projects.env file already exists"
        if ! prompt_yes_no "Overwrite existing projects.env file?" "n"; then
            log_info "Keeping existing projects.env file"
            return 0
        fi
    fi

    if [ ! -f "$PROJECTS_EXAMPLE" ]; then
        log_error "projects.env.example not found"
        exit 1
    fi

    echo ""
    log_info "Enter your project names (space-separated)"
    log_info "These should match the prefix of your container names"
    log_info "Example: If containers are 'projecta-backend' and 'projectb-api'"
    log_info "         Enter: projecta projectb"
    echo ""

    PROJECTS=$(prompt_input "Project names" "projecta projectb")
...
EOF
    ...
    log_success "Created projects.env file with projects: $PROJECTS"
}
...
validate_setup() {
    ...
    # Check projects.env
    if [ ! -f "$PROJECTS_FILE" ]; then
        log_error "projects.env file not found"
        errors=$((errors + 1))
    fi
...
show_next_steps() {
    ...
    echo ""
    printf "  1. ${CYAN}Update DNS${NC}\n"
    printf "     Point your syslog subdomains to this server:\n"
    # shellcheck disable=SC1090
    [ -f "$PROJECTS_FILE" ] && . "$PROJECTS_FILE"
    for project in $PROJECTS; do
        printf "       syslog.%s.com -> <server-ip>\n" "$project"
    done
    echo ""
    printf "  2. ${CYAN}Review Caddyfile${NC}\n"
    printf "     Ensure domain blocks match your projects:\n"
    printf "       nano Caddyfile\n"
    echo ""
    printf "  3. ${CYAN}Start services${NC}\n"
    printf "       docker compose up -d\n"
    echo ""
    printf "  4. ${CYAN}Run bootstrap${NC}\n"
    printf "       ./bootstrap.sh\n"
    echo ""
    printf "  5. ${CYAN}Verify health${NC}\n"
    printf "       docker compose ps\n"
    echo ""
    printf "  6. ${CYAN}Delete credentials file${NC}\n"
